...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def updateData(self):
        ...
        npm = self.lineEditNPM.text()
        kelas = self.lineEditKelas.text()
        mataprak = self.lineEditMataPrak.text()
        strsql = "UPDATE tbl_mahasiswa SET npm='"+npm+"', kelas='"+kelas+"', mata_prak='"+mataprak+"' WHERE nama='"+nama+"'"
        conn = conndb.conndb()
        conn.queryExecute(strsql)
        self.loadData()
